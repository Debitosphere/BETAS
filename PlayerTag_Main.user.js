    ...
    function crPlayerTag_Main() {
        ...
        function CityPlayerTagInclude() {
			...
			if (matchList == undefined ) {
				matchList = webfrontend.gui.region.RegionCityMenu.prototype.onTick.toString()
						.match(/if\(this\.([A-Za-z0-9_]+)!==null\)this\.[A-Za-z0-9_]+\(\);/);
						console.log("check 7");
			}
			//selectedObjectMemberName = matchList[1];
			selectedObjectMemberName = webfrontend.gui.region.RegionCityMenu.prototype.onTick.toString()
            ...
            PT.util.ajax('GET', url, null, function(xhr){
                            if (xhr.responseText !=null && xhr.responseText != ""){
								...
								//var res = [...new Set(tagArray.map(x => x.Coords))];
								//tagArray = res;
                                localStorage.setItem(localStorageKey, JSON.stringify(tagArray));
                            }
             });
         ...
         function updateFromServer(){
            ...
            PT.util.ajax('GET', url, null, function(xhr){
                            if (xhr.responseText !=null && xhr.responseText != ""){
                                ...
                                tagArray = JSON.parse(xhr.responseText);
								//res = [...new Set(tagArray.map(x => x.Coords))];
								//tagArray = res;
        ...
        PT.util = {
            ...
            urlGet: function(world, alliance) {
                return PT.util.URLGET;
				//return PT.util.URLGET + "?world=" + world + "&alliance="+alliance;
            },
